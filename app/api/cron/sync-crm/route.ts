...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const integration of integrations) {
      try {
        ...
        const contacts = await adapter.getContacts({
          limit: 100,
          updated_since: tenMinutesAgo.toISOString()
        })
        ...
        // Проверяем новые успешные сделки для отправки в Facebook
    ...
    return NextResponse.json(
      { error: 'CRM sync failed', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
